...
import type { PurchaseItemInput } from "@/hooks/use-purchases";
...
interface PurchaseItemsEditorProps {
  ...
  onItemsChange: (items: PurchaseItemInput[]) => void;
  products: Product[];
}

export function PurchaseItemsEditor({ items, onItemsChange, products }: PurchaseItemsEditorProps) {
  const runningTotalBOB = items.reduce((sum, item) => sum + item.quantity * item.unitCostBOB, 0);
      ...
      items.map((item, itemIndex) =>
        itemIndex === index
          ? {
              ...
              unitCostBOB: product?.costPriceBOB ?? item.unitCostBOB,
            }
          : item,
      ),
    );
  ...
  const handleUnitCostChange = (index: number, unitCostBOB: number) => {
    onItemsChange(
      items.map((item, itemIndex) => (itemIndex === index ? { ...item, unitCostBOB } : item)),
    );
  };
        ...
        <Table>
          ...
          <TableBody>
            {items.map((item, index) => {
              const subtotalBOB = item.quantity * item.unitCostBOB;
                ...
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Input
                      ...
                      step="0.01"
                      className="w-28"
                      value={item.unitCostBOB}
                      onChange={(event) => handleUnitCostChange(index, Number(event.target.value))}
                    />
                  </TableCell>
